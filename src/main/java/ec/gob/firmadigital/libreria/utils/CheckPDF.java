        ...
        if (namesDictionary != null && !namesDictionary.isEmpty()) {
            ...
            if (pdfObject != null) {
                ...
                if (pdfObject.isDictionary()) {
                    getJavaScriptFromPdfDictionary((PdfDictionary) pdfObject, strBuf);
                } else if (pdfObject.isArray()) {
                    getJavaScriptFromPdfArray((PdfArray) pdfObject, strBuf);
                } else if (pdfObject.isString()) {
                    strBuf.append(((PdfString) pdfObject).getValue());
                }
            ...
            if (javascriptDictionary != null) {
                ...
                for (Map.Entry<PdfName, PdfObject> entry : set) {
                    ...
                    if (pdfObj.isDictionary()) {
                        getJavaScriptFromPdfDictionary((PdfDictionary) pdfObj, stringBuilder);
                    } else if (pdfObj.isArray()) {
                        getJavaScriptFromPdfArray((PdfArray) pdfObj, stringBuilder);
                    } else if (pdfObj.isString() && entry.getKey().getValue().equals(PdfName.JS.getValue())) {
                        stringBuilder.append(((PdfString) pdfObj).getValue());
                    }
                }
            ...
            if (objs != null) {
                ...
                for (Map.Entry<String, PdfObject> entry : objs.entrySet()) {
                    ...
                    if (pdfObject.isDictionary()) {
                        ...
                        getJavaScriptFromPdfArray((PdfArray) pdfObject, stringBuilder);
                    } else if (pdfObject.isString() && entry.getKey().equals(PdfName.JS.getValue())) {
                        stringBuilder.append(((PdfString) pdfObject).getValue());
                    }
                }
                checkPDF += "\nJAVASCRIPT NAMED TREE:\n" + stringBuilder.toString().trim();
            }
        }
